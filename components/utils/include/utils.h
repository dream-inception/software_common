 ...
 *    - ESP_FAIL
 */
esp_err_t reboot(TickType_t wait_ticks);
 ...
 *     - count
 */
int reboot_unbroken_count(void);
 ...
 *     - false
 */
bool reboot_is_exception(bool erase_coredump);
 ...
 *      - ESP_OK
 */
esp_err_t timesync_wait(uint32_t wait_ms);
